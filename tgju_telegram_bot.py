...
# ارزهای دیجیتال - قیمت به دلاره، با ضرب در نرخ دلار به تومان تبدیل میشه
CRYPTO_ITEMS = [
...
RTL = "\u200f"  # کاراکتر راست‌به‌چپ برای قسمت ارزها
...
def to_toman(price_str: str) -> str:
    """قیمت ریالی rو به تومان تبدیل می‌کنه."""
...
def usd_to_toman(usd_str: str, dollar_toman: float) -> str:
    """قیمت دلاری رو به تومان تبدیل می‌کنه."""
...
def build_message(prices: dict) -> str:
    # نرخ دلار به تومان برای تبدیل کریپتو
    dollar_toman = 0.0
    ...
    for pid, flag in CURRENCY_ITEMS:
        if pid in prices:
            # RTL در ابتدای هر خط، ارزها رو از راست به چپ نشون میده
            lines.append(f"{RTL}{flag} {to_toman(prices[pid])}")
    ...
    for pid, fa_name in CRYPTO_ITEMS:
        if pid in prices:
            if dollar_toman > 0:
                value = usd_to_toman(prices[pid], dollar_toman)
            else:
                value = prices[pid]
            lines.append(f"{fa_name}: {value}")
